local function setrank(msg, name, value) -- setrank function
 ...
 return send_msg('chat#id'..msg.to.id, '???? ????? ('..name..') ?? '..value..' ????? ???? ?? ', ok_cb,  true)
  end
end
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '?????'
    ...
    local text = '??? ???? : '..(result.first_name or '')..' '..(result.last_name or '')..'\n'
               ..'????: '..Username..'\n'
               ..'???? ?????? : '..result.id..'\n\n'
 ...
 local value = redis:hget(hash, result.id)
  ...
  text = text..'????? ???? ??? ??????? ??? : '..user_info_msgs..'\n\n'
  text = text..'@Mr_Vigeo'
  send_msg(extra.receiver, text, ok_cb,  true)
  else
 send_msg(extra.receiver, extra.user..' ??? ?????? ???? ??? ???? ???.', ok_cb, false)
  end
end
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  if result.from.username then
    ...
    local text = '??? ???? : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
               ..'????: '..Username..'\n'
               ..'???? ?????? : '..result.from.id..'\n\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = '??? : '..(msg.from.first_name or '?????')..'\n'
   local text = text..'????? : '..(msg.from.last_name or '?????')..'\n' 
   local text = text..'???? : '..Username..'\n'
   local text = text..'???? ?????? : '..msg.from.id..'\n\n'
